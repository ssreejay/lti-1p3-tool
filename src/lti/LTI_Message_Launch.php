...
class LTI_Message_Launch
{
    ...
    public function validate(array $request = null)
    {
        ...
        // 1) Validate the 'state'. If it fails, might fallback to postMessage.
        //    But do not throw an error or call exit if fallback is triggered.
        //    Instead, store the token data if we have it.


        // 2) If we do have an id_token in $request, proceed with the rest:
        if (!empty($request['id_token'])) {
            $this->validate_jwt_format()
                ->validate_nonce()
            ...
            // 3) Cache the launch data (the JWT) so future requests can skip
            //    re-checking the id_token.
            $this->cache_launch_data();
            // save to database
            $this->db->store_lti_launch($this->launch_id, $this->jwt['body']);
        }
        $this->validate_state();
    ...
    public function validate_old(array $request = null)
    {
        ...
        foreach ($public_key_set['keys'] as $key) {
            if ($key['kid'] == $this->jwt['header']['kid']) {
                try {
                    ...
                    return openssl_pkey_get_details(openssl_pkey_get_public($parsedKey->getKeyMaterial()));
                } catch (\Exception $e) {
                    return false;
                }
            }
    ...
    private function validate_state_old()
    {
    ...
    {
        // Gather state from POST or GET
        $providedState = $this->request['state'] ?? $_GET['state'] ?? '';

        // 1) If we have a matching cookie, skip fallback
        if (!empty($providedState)) {
            $cookieKey = 'lti1p3_' . $providedState;
            if ($this->cookie->get_cookie($cookieKey) === $providedState) {
                return $this;
            }
        }

        // 2) Fallback to postMessage
        $this->fallback_to_postmessage();
        exit();
    }



    private function fallback_to_postmessage()
    {
        $expectedState = $this->request['state'];
        $expectedLaunchID = $this->launch_id;

        $state = bin2hex(random_bytes(16));
        require_once(__DIR__ . '/../../../../../utilities/database.php');
        $database = new \LtiDatabase();
        $database->store_state($state, [
        ...
        <html>
        <head>
            <title>Verifying LTI Launch</title>
        </head>
        <body>
            <script>
                (function() {
                    ...
                    console.log('Posting get_data message:', JSON.stringify(message));
                    ...
                    window.addEventListener('message', function(event) {
                        ...
                        if (event.data.subject === 'lti.get_data.response') {
                            var storedValue = event.data.value;
        
                            if (storedValue === expectedState) {
                                // Verified via postMessage. Let's call the server
                                console.log('State verified via postMessage');
        
                                // Redirect with verified parameters
                                window.location = '?post_message_verified=1&state=' + encodeURIComponent(redirectState);
                            } else {
                                console.error('State mismatch! Possible security risk.');
                                document.body.innerHTML = '<h3>State mismatch. Please try again or contact support.</h3>';
                            }
                        }
                    });
