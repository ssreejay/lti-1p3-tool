...
class LTI_OIDC_Login
{
     ...
     */
    public function do_oidc_login_redirect_old($launch_url, array $request = null)
    {
     ...
     */
        ...
        // Set state in cookie (fallback for non-Safari browsers)
        $this->cookie->set_cookie("lti1p3_$state", $state, 60);
        ...
        // Add JavaScript to store the state using postMessage
        echo "
        <script>
            function storeLtiState() {
                ...
                console.log('Sending put_data message:', msg);
                window.parent.postMessage(msg, '*');
        
                window.addEventListener('message', function(event) {
                    if (event.data.subject === 'lti.put_data.response') {
                        if (event.data.error) {
                            console.error('Error in lti.put_data:', event.data.error);
                        } else {
                            console.log('lti.put_data was successful!');
                        }
                    }
                });
        
                setTimeout(() => {
                    window.location.href = '{$registration->get_auth_login_url()}?" . http_build_query($auth_params) . "';
                }, 1000);
            }
        
            window.onload = storeLtiState;
        </script>
        ";

        exit(); // Stop execution so that JS runs
    }
